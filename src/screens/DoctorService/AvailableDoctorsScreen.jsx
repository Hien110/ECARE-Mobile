        ...
        <View style={styles.infoBlock}>
          ...
          {rating != null ? (
            <View style={styles.ratingRow}>
              ...
              <Text style={styles.ratingText}>{` ${Number(rating).toFixed(
                1,
              )} (${ratingCount || 0})`}</Text>
            </View>
          ) : ratingCount === 0 ? (
...
const AvailableDoctorsScreen = ({ navigation, route }) => {
  const { scheduledDate, slot, specialization, elderly, family } =
    route.params || {};
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.container}>
        <Text style={styles.hint}>
          Ngày: {scheduledDate} • Ca: {slotLabelDisplay} • {slotTime}
        </Text>
        ...
        {loading ? (
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <DoctorItem item={item} onPress={handleSelect} />
            )}
