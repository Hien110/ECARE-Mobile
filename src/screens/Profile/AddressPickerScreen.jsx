...
import { View, Text, StyleSheet, TouchableOpacity, TextInput, ScrollView, StatusBar, Alert, Modal, FlatList, SafeAreaView, ActivityIndicator } from "react-native"
...
const AddressPickerScreen = ({ navigation }) => {
    ...
    try {
      ...
      if (apiResult.success && apiResult.data) {
        ...
        if (apiResult.data.currentAddress) {
          // Parse existing address to extract street address
          const addressParts = apiResult.data.currentAddress.split(',')
          if (addressParts.length > 0) {
            setAddress(addressParts[0].trim())
          }
          
          // Try to extract and set commune if possible
  ...
  const getCoordinatesFromAddress = async (fullAddress) => {
    try {
      ...
      // Default Đà Nẵng coordinates (city center)
      let baseCoordinates = {
        latitude: 16.0471,
        longitude: 108.2068
      }
      ...
      if (selectedCommuneObj) {
        // Add small random offset based on commune code to simulate different locations
      ...
      // Try external geocoding as enhancement (but don't fail if it doesn't work)
      try {
        ...
        console.log('Geocoding failed, using base coordinates:', geoError.message)
      }
  ...
  const handleGetCoordinates = async () => {
    if (!address.trim() || !selectedCommune) {
      Alert.alert("Thông báo", "Vui lòng điền đầy đủ thông tin địa chỉ trước khi lấy tọa độ")
      return
    }

    const selectedCommuneObj = communes.find(c => c.code === selectedCommune)
    const fullAddress = `${address.trim()}, ${selectedCommuneObj?.name || ''}, Đà Nẵng, Việt Nam`
  ...
  const extractCommuneFromAddress = (fullAddress, communesList) => {
    ...
    if (addressParts.length > 1) {
      ...
      // Try to find matching commune by name
      const matchingCommune = communesList.find(commune => 
  ...
  const fetchCommunes = async () => {
    setLoading(true)
    try {
      const response = await fetch('https://production.cas.so/address-kit/2025-07-01/provinces/48/communes')
      if (!response.ok) {
      ...
      // After loading communes, try to extract commune from current address
  ...
  // Extract commune when both currentUser and communes are available
  useEffect(() => {
    if (currentUser?.currentAddress && communes.length > 0 && !selectedCommune) {
  ...
  const selectCommune = (commune) => {
    ...
    // Reset coordinates when commune changes
  ...
  const handleSaveAddress = async () => {
    ...
    setSaving(true)
    try {
      // Get selected commune name for full address
      const selectedCommuneObj = communes.find(c => c.code === selectedCommune)
      const fullAddress = `${address.trim()}, ${selectedCommuneObj?.name || ''}, Đà Nẵng`
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.content}>
        ...
        {currentUser?.currentAddress && (
          <View style={styles.currentAddressSection}>
            ...
            <View style={styles.currentAddressContainer}>
              ...
              <Text style={styles.currentAddressText}>{currentUser.currentAddress}</Text>
            </View>
          </View>
        )}
        ...
        <View style={styles.addressSection}>
          ...
          {/* Province/City - Fixed as Đà Nẵng */}
          <View style={styles.inputGroup}>
            <Text style={styles.inputLabel}>
          ...
          <View style={styles.inputGroup}>
            ...
            >
              {loading ? (
                ...
                <>
                  <Text style={[styles.dropdownText, !selectedCommune && styles.placeholderText]}>
            ...
            <View style={styles.coordinatesSection}>
              <TouchableOpacity 
                style={[
                  ...
                  (!address.trim() || !selectedCommune) && styles.coordinatesButtonDisabled
                ]} 
              ...
              >
                ...
                <Text style={styles.coordinatesButtonText}>
                  {loadingCoordinates ? "Đang lấy tọa độ..." : "Lấy tọa độ địa chỉ"}
                </Text>
              </TouchableOpacity>
              {addressCoordinates && (
                <View style={styles.coordinatesInfo}>
                  <Text style={styles.coordinatesText}>
                    📍 Lat: {addressCoordinates.latitude.toFixed(6)}, Lng: {addressCoordinates.longitude.toFixed(6)}
                  </Text>
                </View>
              )}
              ...
              <FlatList
                ...
                ListEmptyComponent={
                  <View style={styles.emptyContainer}>
                    <Text style={styles.emptyText}>Không có dữ liệu xã/phường</Text>
                  </View>
                }
              />
            )}
